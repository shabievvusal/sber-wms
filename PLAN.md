...
## Важное уточнение к переносу — таблицы статистики нужно создать даже без данных (2026-07-16)

При развёртывании на СЕРВЕРЕ (тот же перенос, что и локально, см. выше)
всплыла ошибка `42P01: relation "wms_ops" does not exist` в dotnet-логах.
Причина: `wms_ops`/`wms_placement`/`wms_receiving`/`wms_remains`/
`product_weights`/`wms_storage_agg` создаются НЕ автоматически при старте
Node (в отличие от `employees`/`routes`/`tsd_*`/`vs_*`, у которых есть
`init()` в соответствующих `*-pg.js`, вызываемый на каждом старте) — эти
6 таблиц создаёт ТОЛЬКО одноразовый `migrate-storage-json-to-pg.js`,
который мы намеренно не запускали (статистику/смены сознательно не
переносили — дороже места на диске, чем пользы, легче дособрать заново).
Без запуска скрипта хотя бы раз — таблиц нет вообще, и dotnet падает с
`42P01` на любой запрос статистики, даже с историей операций в 0 строк.

- **Исправление**: запустить `migrate-storage-json-to-pg.js` даже когда
  в `backend/data/` нет ни одной папки-даты — скрипт устроен так, что
  сначала безусловно создаёт схему (`createSchema()`), и только потом
  ищет данные для переноса; при 0 найденных дат просто завершается,
  оставляя таблицы пустыми. Именно это и нужно — таблицы есть, реальных
  исторических данных нет, копится заново по мере использования.
- **Вывод на будущее**: при любом новом развёртывании этого стека (не
  только этом) — `migrate-storage-json-to-pg.js` нужно запускать ВСЕГДА
  хотя бы раз (создать схему), даже если решено не переносить сами
  данные статистики.

## Пустая ячейка «Компания» в сводке вместо «—» — JS `||` vs C# `??` (готово, 2026-07-16)

Пользователь: «в своде по компании пустая ячейка в столбце компания
вместо "Не в списке" или "Без компании"... Вроде прочерк». Сверка с
оригиналом (`backend/storage.js:631`: `const c = r.company || '—';`)
подтвердила — да, в оригинале именно прочерк `—`, подставляется на
бэкенде, а не на фронтенде.

Причина расхождения — классическая JS→C# ловушка при переносе:
`ResolveCompany()` в `StatsService.cs` возвращал `""` (пустую строку, не
`null`) для сотрудников, у которых в реестре ЕСТЬ executorId, но НЕ
назначена компания (`company = ''` в таблице `employees` — те самые
«сотрудники без компании», см. фикс `EmployeesCard.jsx` от 2026-07-15).
Все вызывающие места делали `ResolveCompany(...) ?? "—"` — а C#'s `??`
ловит только буквальный `null`, в отличие от JS'ного `||` у оригинала,
который считает пустую строку тоже «ложной» и подставляет `'—'`. Пустая
строка спокойно проходила мимо `?? "—"` и долетала до фронтенда как есть
— отсюда и пустая (не прочерк, не «Без компании») ячейка.

- [x] `backend-dotnet/Services/StatsService.cs` — `ResolveCompany()`
      исправлен у источника: `idMap.TryGetValue(executorId, out var c)
      && !string.IsNullOrEmpty(c) ? c : null` (было — без проверки
      `IsNullOrEmpty`). Все ~9 мест, где результат используется через
      `?? "—"` (сводка по компаниям, employee-строки для «По СЗ»/«По
      зонам», фильтрация по списку компаний и т.д.), получили исправление
      сразу, без правки каждого по отдельности — один источник правды.
      Три места, которые уже сами проверяли `string.IsNullOrEmpty(company)`
      без `?? "—"` (фильтрация по разрешённым компаниям в
      placement/receiving/remains) — поведение не изменилось, они и
      раньше одинаково относились к `null`/`""`.
- **Верификация**: `docker compose build dotnet` — чисто, передеплоено
  локально (`up -d --force-recreate dotnet`). Живую проверку с реальным
  сотрудником без компании в браузере не проводил — логика проверена
  чтением кода (единственная точка, где рождалась пустая строка, теперь
  гарантированно даёт `null` → `—` на всех уровнях).
